#import "HBNQRootListController.h"
#import "HBNQHeaderView.h"
...
static NSString *kTweakName = @"NotiQuiet";
static NSString *kAuthorName = @"HASHBANG PRODUCTIONS";
static NSString *kTwitterString = @"I'm using NotiQuiet by @hbangws to silence notifications when I'm in specific apps. Available free in the BigBoss repo!";

@interface HBNQRootListController () <MFMailComposeViewControllerDelegate> {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
	return section == 0 ? [[HBNQHeaderView alloc] initWithTweakName:kTweakName author:kAuthorName] : nil;
} 
...
- (void)twitterButtonTapped:(UIBarButtonItem *)sender {
    if ([SLComposeViewController isAvailableForServiceType:SLServiceTypeTwitter]) {
        ...
        [composeController setInitialText:kTwitterString];
...
- (void)emailHashbangSupport {
    ...
    [emailHB setSubject:[NSString stringWithFormat:@"%@ Support", kTweakName]];
    ...
    [emailHB addAttachmentData:[NSData dataWithContentsOfFile:@"/var/mobile/Library/Preferences/ws.hbang.notiquiet.plist"] mimeType:@"application/xml" fileName:@"NotiQuietPrefs.plist"];
