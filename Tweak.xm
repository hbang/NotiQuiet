...
-(void)alertView:(UIAlertView *)alert clickedButtonAtIndex:(NSInteger)index{
	if([[alert buttonTitleAtIndex:index]isEqualToString:__(@"WELCOME_SETTINGS")])[[%c(SpringBoard) sharedApplication]applicationOpenURL:[NSURL URLWithString:@"prefs:root=NotiQuiet"] publicURLsOnly:NO]; //damn you 5.1
}
@end
...
-(void)observer:(id)observer addBulletin:(id)bulletin forFeed:(unsigned)feed{
	...
	if(([prefs objectForKey:@"Enabled"]&&![[prefs objectForKey:@"Enabled"]boolValue])
		...
		&&[[prefs objectForKey:[@"App-" stringByAppendingString:currentApp]]boolValue]))%orig;
}
%end
%hook SBUIController
...
static void ADNQPrefsUpdate(CFNotificationCenterRef center,void *observer,CFStringRef name,const void *object,CFDictionaryRef userInfo){
	ADNQPrefsLoad();
}
